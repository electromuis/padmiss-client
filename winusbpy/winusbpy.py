...
class WinUsbPy(object):
	...
	def list_usb_devices(self, **kwargs):
		...
		try:
			if kwargs["default"] == True:
				value = value | DIGCF_DEFAULT
			if kwargs["present"] == True:
				value = value | DIGCF_PRESENT
			if kwargs["allclasses"] == True:
				value = value | DIGCF_ALLCLASSES
			if kwargs["profile"] == True:
				value = value | DIGCF_PROFILE
			if kwargs["deviceinterface"] == True:
				value = value | DIGCF_DEVICE_INTERFACE
		except KeyError:
			if value == 0x00000000:
				value = 0x00000010
			pass

		value = DIGCF_PRESENT | DIGCF_DEVICE_INTERFACE
		flags = DWORD(value)
	...
	def init_winusb_device(self, vid, pid):
		...
		try:
			path = list(filter(self.find_device, self.device_paths))[0]
		except IndexError:
			return False

		self.init_winusb_device_by_path(path)



	def init_winusb_device_by_path(self, path):
